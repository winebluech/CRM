    ...
    if (opts.pan) {
      ...
      $chart.on('mousedown touchstart',function(e){
		...
		if(e.type === "touchstart" || e.type === "touchmove" ){
			startX=e.originalEvent.targetTouches[0].pageX-lastX;
			startY=e.originalEvent.targetTouches[0].pageY-lastY;
		}
        ...
        $(document).on('mousemove touchmove',function(ev) {
		  ...
		  if(ev.type === "touchstart" || ev.type === "touchmove" ){
			newX = ev.originalEvent.targetTouches[0].pageX - startX;
					
			newY = ev.originalEvent.targetTouches[0].pageY - startY;
			}
          var lastTf = $this.css('transform');
		  
          if (lastTf === 'none') {
			  console.log(ev.type+"2"+lastTf);
            if (lastTf.indexOf('3d') === -1) {
